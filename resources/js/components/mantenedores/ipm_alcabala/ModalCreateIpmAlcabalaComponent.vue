<template>
    <div class="modal fade xd2" id="modalCrearIpmAlcabala"  tabindex="-1" role="dialog"
          data-keyboard="false" data-backdrop="static">
          <page-loader v-if="loading==true"/>
        <div class="modal-dialog " role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title w-100 text-center">CREAR IPM ALCABALA</h5>
                </div>
                <div class="modal-body">
                    <form  @submit.prevent="guardarIpmAlcabala">
                        <div class="row">
                            <div class="mb-3 col-12 col-md-3">
                                <label class="form-label" for="lname">Año:</label>
                                <select id="2" class="form-control" v-model="ipm_alcabala.anio" >
                                    <option value="" :selected="true">Seleccionar:</option>
                                    <option v-for="(year, index) in years" :key="index" :value="year">{{ year }}</option>
                                </select>
                                <div v-if="enviado && !$v.ipm_alcabala.anio.required" class="mensajeError">
                                    Debe seleccionar el año.
                                </div>
                            </div>
                            <div class="mb-3 col-12 col-md-3">
                                <label class="form-label" for="lname">Mes:</label>
                                <select id="2" class="form-control" v-model="ipm_alcabala.mes" >
                                    <option value="" :selected="true">Seleccionar:</option>
                        ...
                        <div class="row">
                            ...
                            <div class="mb-3 col-12 col-md-4" style="margin-top:2%">
                                ...
                                <button type="button"
                                    class="btn btn-danger btn-pill"  @click="closeModalAgregarIpmAlcabalaOut()"
        ...
        methods:{
            guardarIpmAlcabala(){
                ...
                if (this.$v.$invalid) {
                    ...
                    formData.append('anio',this.ipm_alcabala.anio)
                    formData.append('mes',this.ipm_alcabala.mes)
                    formData.append('mensual',this.ipm_alcabala.mensual)
                    formData.append('acumulado',this.ipm_alcabala.acumulado)
                    var request ={
                        url:`${this.variableGlobal}/store-data-ipm-alcabala`,
                        method:'post',
                        data:formData
                    }
                    ...
                    this.loading=false;
                    this.closeModalAgregarIpmAlcabala();
                }
            },
            closeModalAgregarIpmAlcabala(){
                this.limpiarIpmAlcabala();
                $('#modalCrearIpmAlcabala').modal('hide');
                Bus.$emit("DetalleCreateIpmAlcabala");
            },
            closeModalAgregarIpmAlcabalaOut(){
                this.limpiarIpmAlcabala();
                $('#modalCrearIpmAlcabala').modal('hide');
            },
            limpiarIpmAlcabala(){
                    this.ipm_alcabala.anio='',
                    this.ipm_alcabala.mes=''
                    this.ipm_alcabala.mensual='',
                    this.ipm_alcabala.acumulado=''
            },
            getMeses(){
